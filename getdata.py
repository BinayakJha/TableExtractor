    ...
    if table := page.extract_table(): # extract_table() returns None if there are no tables
        ...
        for i, table in enumerate(tables):
            if output_format == "csv":
                table.to_csv(f'tables/table{str(page.page_number)}_{str(i)}.csv', index=False)
                print(f'tables/table{str(page.page_number)}_{str(i)}.csv')
            elif output_format == "excel":
                table.to_excel(f'tables/table{str(page.page_number)}_{str(i)}.xlsx', index=False)
                print(f'tables/table{str(page.page_number)}_{str(i)}.xlsx')
            else:
